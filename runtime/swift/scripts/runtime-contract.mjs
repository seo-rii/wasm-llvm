...
export function validateSwiftRuntimeContract(contract) {
	...
	if (!Array.isArray(contract.requiredWorkerRequestFields)) {
		...
		contract.requiredWorkerRequestFields.length !== SWIFT_REQUIRED_WORKER_REQUEST_FIELDS.length ||
	...
	if (!Array.isArray(contract.requiredWorkerResponseFields)) {
		...
		contract.requiredWorkerResponseFields.length !== SWIFT_REQUIRED_WORKER_RESPONSE_FIELDS.length ||
	...
	for (const [index, testCase] of contract.cases.entries()) {
		...
		if (
			...
			try {
				...
				errors.push(`cases[${index}].expectedErrorPattern must be a valid regular expression`);
			}
		}
		if (
