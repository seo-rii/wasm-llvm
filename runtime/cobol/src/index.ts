...
const DEFAULT_COBOL_RUNTIME_BASE_URL = new URL('../assets/', import.meta.url);
...
export interface CreateCobolCompilerOptions {
...
function normalizeBaseUrl(baseUrl: string | URL = DEFAULT_COBOL_RUNTIME_BASE_URL) {
	const value = baseUrl.toString();
	return new URL(value.endsWith('/') ? value : `${value}/`, import.meta.url);
}
...
export function resolveCobolRuntimeManifestUrl(
	baseUrl: string | URL = DEFAULT_COBOL_RUNTIME_BASE_URL
) {
...
export async function loadCobolRuntimeManifest(
	manifestUrl: string | URL = resolveCobolRuntimeManifestUrl(),
	fetchImpl: typeof fetch = fetch
): Promise<CobolRuntimeManifestV1> {
	const url = manifestUrl instanceof URL ? manifestUrl : new URL(manifestUrl, import.meta.url);
	if (url.protocol === 'file:') {
		const [{ readFile }, { fileURLToPath }] = await Promise.all([
			import('node:fs/promises'),
			import('node:url')
		]);
		return parseCobolRuntimeManifest(JSON.parse(await readFile(fileURLToPath(url), 'utf8')));
	}
...
class CobolCompiler implements BrowserCobolCompiler {
	...
	static async create(options: CreateCobolCompilerOptions) {
		...
		const manifest =
			...
			(await loadCobolRuntimeManifest(
				resolveCobolRuntimeManifestUrl(options.runtimeBaseUrl),
				fetchImpl
			));
		const assets = resolveCobolRuntimeAssetUrls(options.runtimeBaseUrl, manifest);
		const clangManifest =
			options.clangManifest ||
			(await (options.clangRuntimeBaseUrl
				? loadRuntimeManifest(
						resolveRuntimeManifestUrl(options.clangRuntimeBaseUrl),
						fetchImpl
					)
				: loadRuntimeManifest(undefined, fetchImpl)));
		const runtime = new BrowserClangRuntime({
			runtimeBaseUrl: options.clangRuntimeBaseUrl,
			manifest: {
				...clangManifest,
				compiler: {
	...
	async compile(request: BrowserCobolCompileRequest): Promise<BrowserCobolCompilerResult> {
		...
		try {
			...
			emitProgress(request, 'compile', 55, 'compiling generated C with wasm-llvm');
