...
export async function validateRustRuntimeProfile(sourceDir: string) {
  ...
  if (!manifestStats?.isFile()) {
    throw new Error(
      `Rust LLVM runtime manifest was not found at ${manifestPath}`,
    );
  }
  const manifest = JSON.parse(await readFile(manifestPath, "utf8")) as {
    manifestVersion?: number;
    version?: string;
    producer?: { id?: string; manifestSha256?: string; runner?: string };
    compiler?: { rustcWasm?: string };
    targets?: Record<
      string,
      {
        sysrootPack?: { asset?: string; index?: string };
        compile?: {
          kind?: string;
          llvm?: {
            llc?: string;
  ...
  if (targets.length === 0)
    throw new Error("Rust runtime profile does not define any targets");
  ...
  if (integratedTargets.length > 0) {
    if (integratedTargets.length !== targets.length) {
      throw new Error(
        "Rust runtime profile cannot mix integrated and split LLVM targets",
      );
    }
    if (manifest.version !== `rust-${RUST_INTEGRATED_PROFILE.rustVersion}`) {
      throw new Error(
        `Rust runtime profile requires ${RUST_INTEGRATED_PROFILE.rustVersion}`,
      );
    }
    if (
      manifest.producer?.id !== "@seo-rii/wasm-llvm/rust-browser" ||
      !manifest.producer.manifestSha256?.match(/^[0-9a-f]{64}$/) ||
      (manifest.producer.runner !== "host" &&
        manifest.producer.runner !== "container")
    ) {
      throw new Error(
        "Rust integrated profile has invalid producer provenance",
      );
    }
    for (const [target, config] of targets) {
      ...
      for (const relativePath of [
        config.sysrootPack.asset,
        config.sysrootPack.index,
      ]) {
        const fileStats = await stat(
          path.join(sourceDir, "runtime", relativePath),
        ).catch(() => null);
        if (!fileStats?.isFile()) {
          throw new Error(
            `Rust integrated runtime asset was not found: ${relativePath}`,
          );
        }
      }
    }
  ...
  if (manifest.version !== `rust-${RUST_LLVM_PROFILE.rustVersion}`) {
    throw new Error(
      `Rust LLVM profile requires ${RUST_LLVM_PROFILE.rustVersion}`,
    );
  }
  for (const [target, config] of targets) {
    ...
    if (
      ...
      throw new Error(
        `Rust LLVM profile has unexpected LLVM assets for ${target}`,
      );
    }
  }
  for (const relativePath of RUST_LLVM_PROFILE.requiredAssets) {
    const fileStats = await stat(path.join(sourceDir, relativePath)).catch(
      () => null,
    );
    if (!fileStats?.isFile())
      throw new Error(`Rust LLVM asset was not found: ${relativePath}`);
  }
  ...
  if (
    lldStats.some(Boolean) &&
    !lldStats.every((fileStats) => fileStats?.isFile())
  ) {
