...
const CACHE_ROOT =
	process.env.WASM_LLVM_OBJECTIVE_C_CACHE_DIR ||
...
const SOURCE_STAGING_DIR = path.join(BUILD_DIR, 'source');
...
const ROBIN_MAP_URL = PRODUCER_MANIFEST.sources.robinMap.repository;
const ROBIN_MAP_REF = PRODUCER_MANIFEST.sources.robinMap.ref;
const ROBIN_MAP_COMMIT = PRODUCER_MANIFEST.sources.robinMap.commit;
...
async function writeStagedFile(filePath, contents) {
	const targetPath = path.join(SOURCE_STAGING_DIR, filePath);
...
async function installHeaders(relativePath, source) {
	const headers = await collectHeadersForSource(relativePath, source);
	for (const header of headers) {
		await writeStagedFile(header, await readSourceHeader(header));
	}
}


function compileArgsFor(relativePath, sourcePath, objectPath, resourceDir) {
	...
	const systemIncludeArgs = isCxxSource(relativePath)
		? [
				'-internal-isystem',
				path.join(SYSROOT, 'include', target, 'noeh', 'c++', 'v1'),
				'-internal-isystem',
				path.join(SYSROOT, 'include', 'c++', 'v1'),
				'-internal-isystem',
				resourceIncludeDir,
				'-internal-isystem',
				path.join(SYSROOT, 'include', target),
				'-internal-isystem',
	...
	const localIncludeArgs = isCxxSource(relativePath)
		? ['-iquote', SOURCE_STAGING_DIR, '-idirafter', SOURCE_STAGING_DIR]
		: ['-I', SOURCE_STAGING_DIR, '-I', path.join(SOURCE_STAGING_DIR, 'objc')];
	const baseArgs = [
		'-cc1',
		'-triple',
		target,
	...
	return [...baseArgs, ...languageArgs, sourcePath];
}

async function compileObject(relativePath, resourceDir) {
	const source =
	...
	await installHeaders(relativePath, source);
	const sourcePath = await writeStagedFile(relativePath, source);
	const objectName = objectNameFor(relativePath);
	const objectPath = path.join(OBJECT_DIR, objectName);
	await run(CLANG, compileArgsFor(relativePath, sourcePath, objectPath, resourceDir), {
		cwd: SOURCE_STAGING_DIR
	});
	if (!(await exists(objectPath))) throw new Error(`${relativePath} did not produce an object file`);
	return objectPath;
}
...
async function writeHeadersManifest() {
	...
	await writeFile(
		path.join(OUTPUT_DIR, 'headers.json'),
...
await ensureGitCheckout(ROBIN_MAP_DIR, ROBIN_MAP_URL, ROBIN_MAP_REF, ROBIN_MAP_COMMIT);
await rm(BUILD_DIR, { recursive: true, force: true });
await mkdir(OBJECT_DIR, { recursive: true });
await mkdir(SOURCE_STAGING_DIR, { recursive: true });
await mkdir(OUTPUT_DIR, { recursive: true });
const clangResourceDir = await run(CLANG, ['-print-resource-dir'], { capture: true });

const objectPaths = [];
for (const sourceFile of sourceFiles) {
	console.log(`[objective-c] compiling ${sourceFile}`);
	objectPaths.push(await compileObject(sourceFile, clangResourceDir));
}

const archivePath = path.join(OUTPUT_DIR, 'libobjc.a');
...
const clangVersion = (await run(CLANG, ['--version'], { capture: true })).split(/\r?\n/, 1)[0];
const producerManifestBytes = await readFile(path.join(PRODUCER_ROOT, 'manifest.json'));
const outputAssets = {};
for (const asset of ['libobjc.a', 'headers.json']) {
	const bytes = await readFile(path.join(OUTPUT_DIR, asset));
	outputAssets[asset] = {
		bytes: bytes.length,
...
await writeFile(
	path.join(OUTPUT_DIR, 'producer-receipt.json'),
	`${JSON.stringify(
		{
			producer: {
				id: PRODUCER_MANIFEST.producerId,
			...
			toolchain: {
				wasiSdkVersion: PRODUCER_MANIFEST.toolchain.wasiSdkVersion,
				clangVersion
			},
			target: 'wasm32-wasi',
			assets: outputAssets,
			notes: [
				'GNUstep libobjc2 is compiled with single-threaded WASI compatibility shims and the WASI SDK mmap emulation.',
