...
import { finalizeObjectiveCReceipt } from './write-receipt.mjs';
...
const CACHE_ROOT = process.env.WASM_LLVM_OBJECTIVE_C_CACHE_DIR ||
	path.join(os.tmpdir(), 'wasm-llvm-objective-c-producer');
...
const STAGING_ROOT = path.join(BUILD_DIR, 'staging');
...
const WASM_LD = process.env.WASM_LLVM_WASM_LD ||
	(WASI_SDK_PATH ? path.join(WASI_SDK_PATH, 'bin', 'wasm-ld') : null);
const SYSROOT = process.env.WASM_LLVM_WASI_SYSROOT ||
	(WASI_SDK_PATH ? path.join(WASI_SDK_PATH, 'share', 'wasi-sysroot') : null);

const LIBOBJC2_URL = PRODUCER_MANIFEST.sources.libobjc2.repository;
const LIBOBJC2_REF = PRODUCER_MANIFEST.sources.libobjc2.ref;
const LIBOBJC2_COMMIT = PRODUCER_MANIFEST.sources.libobjc2.commit;
const LIBS_BASE_URL = PRODUCER_MANIFEST.sources.gnustepBase.repository;
const LIBS_BASE_REF = PRODUCER_MANIFEST.sources.gnustepBase.ref;
const LIBS_BASE_COMMIT = PRODUCER_MANIFEST.sources.gnustepBase.commit;
...
const traceHeaders = process.env.WASM_LLVM_OBJECTIVE_C_FOUNDATION_TRACE_HEADERS === '1';
const useLibffi = process.env.WASM_LLVM_OBJECTIVE_C_FOUNDATION_USE_LIBFFI === '1';
...
#include <stddef.h>
#ifndef WASM_LLVM_MODE_T_DEFINED
#define WASM_LLVM_MODE_T_DEFINED 1
typedef unsigned int mode_t;
#endif
#ifndef WASM_LLVM_OFF_T_DEFINED
#define WASM_LLVM_OFF_T_DEFINED 1
typedef long long off_t;
#endif
#ifndef WASM_LLVM_SSIZE_T_DEFINED
#define WASM_LLVM_SSIZE_T_DEFINED 1
typedef long ssize_t;
#endif
#ifndef WASM_LLVM_TIME_T_DEFINED
#define WASM_LLVM_TIME_T_DEFINED 1
typedef long time_t;
#endif
typedef int pid_t;
...
#include <sys/types.h>
#ifndef WASM_LLVM_TIMESPEC_DEFINED
#define WASM_LLVM_TIMESPEC_DEFINED 1
...
async function addFileWithDirs(context, filePath, contents) {
	const targetPath = path.join(context.rootDir, filePath);
...
async function resolveProbeHeaderPath(currentPath, includeName) {
	if (includeName.startsWith('ObjectiveC2/')) {
		...
		if (aliasedHeader.startsWith('objc/')) return { path: aliasedHeader, aliasOnly: true };
...
function stagedPathFor(sourcePath) {
...
async function rewriteProbeIncludes(currentPath, source) {
	...
	for (const match of source.matchAll(includePattern)) {
		...
		const replacementPath = resolved ? stagedPathFor(resolved.path) : null;
	...
	if (sourcePath === 'NSString.m') {
		return source
			...
			.replace(
				...
				`
static void
GSWasmLlvmInitializeNSStringCluster(Class self)
{
...
+ (void) initialize
{
  if (NSStringClass == 0)
    {
      GSWasmLlvmInitializeNSStringCluster(self);
    }
}`
			)
			.replace(
...
async function installProbeHeaders(clang, sourcePath, source) {
	...
	const objcHeaders = JSON.parse(
		await readFile(path.join(OUTPUT_DIR, 'headers.json'), 'utf8')
	);
...
async function findArchiver() {
	const candidates = [
		process.env.LLVM_AR,
		WASI_SDK_PATH ? path.join(WASI_SDK_PATH, 'bin', 'llvm-ar') : null,
		path.join(
			REPO_ROOT,
			'artifacts/toolchain-build-yowasp/emsdk/upstream/bin/llvm-ar'
		),
		path.join(
			REPO_ROOT,
			'artifacts/toolchain-build/tools/wasi-sdk-33.0-x86_64-linux/bin/llvm-ar'
		),
		'llvm-ar',
		'ar'
	].filter(Boolean);
...
async function createFoundationCompileRuntime() {
	if (!CLANG || !WASM_LD || !SYSROOT) {
		throw new Error(
			'Set WASI_SDK_PATH, or set WASM_LLVM_CLANG, WASM_LLVM_WASM_LD, and WASM_LLVM_WASI_SYSROOT'
		);
	}
	for (const requiredPath of [CLANG, WASM_LD, SYSROOT, path.join(OUTPUT_DIR, 'headers.json')]) {
		if (!(await exists(requiredPath))) throw new Error(`Objective-C producer input was not found: ${requiredPath}`);
	}
	await ensureGitCheckout(LIBOBJC2_DIR, LIBOBJC2_URL, LIBOBJC2_REF, LIBOBJC2_COMMIT);
	await ensureGitCheckout(LIBS_BASE_DIR, LIBS_BASE_URL, LIBS_BASE_REF, LIBS_BASE_COMMIT);
	if (useLibffi) {
	...
	const { stdout: resourceDir } = await run(CLANG, ['-print-resource-dir'], { capture: true });
	return {
		clang: CLANG,
		wasmLd: WASM_LD,
		sysroot: SYSROOT,
...
async function freshFoundationRuntime(runtime) {
	const rootDir = path.join(STAGING_ROOT, String((stagingSequence += 1)));
	await rm(rootDir, { recursive: true, force: true });
	await mkdir(rootDir, { recursive: true });
	return { ...runtime, rootDir };
}

function foundationCompileArgs(context, sourcePath, objectPath) {
	return [
		'-cc1',
		'-triple',
		'wasm32-wasip1',
		...
		context.resourceDir,
		'-internal-isystem',
		path.join(context.resourceDir, 'include'),
		'-internal-isystem',
		path.join(context.sysroot, 'include', 'wasm32-wasip1'),
		'-internal-isystem',
		path.join(context.sysroot, 'include'),
		'-I',
		context.rootDir,
		'-include',
		'inttypes.h',
		...
		'stdbool.h',
		'-include',
		stagedPathFor('GNUstepBase/preface.h'),
		'-I',
		path.join(context.rootDir, 'Foundation'),
		'-I',
		path.join(context.rootDir, 'GNUstepBase'),
		'-ferror-limit',
		'20',
		'-O0',
		'-ffunction-sections',
...
function libffiCompileArgs(context, sourcePath, objectPath) {
	return [
		'--target=wasm32-wasip1',
		`--sysroot=${context.sysroot}`,
		'-c',
		'-I',
		context.rootDir,
		'-I',
		path.join(context.rootDir, 'include'),
		'-I',
		path.join(context.rootDir, 'src'),
		'-I',
		path.join(context.rootDir, 'src', 'wasm'),
		'-ferror-limit=20',
		'-O2',
		'-ffunction-sections',
		'-fdata-sections',
...
function foundationSupportCompileArgs(context, sourcePath, objectPath) {
	return [
		'--target=wasm32-wasip1',
		`--sysroot=${context.sysroot}`,
		'-c',
...
async function compileFoundationObject(context, sourceName) {
	...
	await installProbeHeaders(context, sourceName, source);
	await addFileWithDirs(context, sourceName, await rewriteProbeIncludes(sourceName, source));
	const objectName = foundationObjectNameFor(sourceName);
	const diskObjectPath = path.join(OBJECT_DIR, objectName);
	await run(context.clang, foundationCompileArgs(context, sourceName, diskObjectPath), {
		cwd: context.rootDir
	});
	if (!(await exists(diskObjectPath))) throw new Error(`${sourceName} did not produce an object file`);
	return diskObjectPath;
}

async function compileFoundationSupportObject(context) {
	await installShimHeaders(context);
	const sourcePath = 'wasm_llvm_foundation_support.c';
	await addFileWithDirs(context, sourcePath, foundationSupportSource);
	const objectName = 'wasm_llvm_foundation_support.o';
	const diskObjectPath = path.join(OBJECT_DIR, objectName);
	await run(
		context.clang,
		foundationSupportCompileArgs(context, sourcePath, diskObjectPath),
		{ cwd: context.rootDir }
	);
	if (!(await exists(diskObjectPath))) throw new Error('foundation support did not produce an object file');
	return diskObjectPath;
}

async function installLibffiCompileHeaders(context) {
	await installShimHeaders(context);
	for (const header of ['ffi.h', 'ffitarget.h', 'ffi_common.h', 'fficonfig.h']) {
		await addFileWithDirs(context, header, shimFiles[header]);
		await addFileWithDirs(context, `include/${header}`, shimFiles[header]);
	}
	await addFileWithDirs(
		context,
...
async function compileLibffiObject(context, sourceName) {
	await installLibffiCompileHeaders(context);
	await addFileWithDirs(
		context,
		sourceName,
	...
	const diskObjectPath = path.join(OBJECT_DIR, objectName);
	await run(context.clang, libffiCompileArgs(context, sourceName, diskObjectPath), {
		cwd: context.rootDir
	});
	if (!(await exists(diskObjectPath))) throw new Error(`${sourceName} did not produce an object file`);
	return diskObjectPath;
...
async function compileSelectedProbeSource(runtime) {
	const context = await freshFoundationRuntime(runtime);
	...
	await installProbeHeaders(context, selectedSource, source);
	await addFileWithDirs(
		context,
		selectedSource,
	...
	const objectPath = path.join(OBJECT_DIR, foundationObjectNameFor(selectedSource));
	await run(context.clang, foundationCompileArgs(context, selectedSource, objectPath), {
		cwd: context.rootDir
	});
...
async function writeFoundationHeadersManifest(makefileSource) {
	...
	await writeFile(
		path.join(OUTPUT_DIR, 'foundation-headers.json'),
...
async function buildFoundationArchive(runtime) {
	...
	const sourceLimit = Number(process.env.WASM_LLVM_OBJECTIVE_C_FOUNDATION_SOURCE_LIMIT || '0');
	...
	const archivePath = path.join(OUTPUT_DIR, 'libgnustep-base.a');
	const prelinkedObjectPath = path.join(OUTPUT_DIR, 'libgnustep-base.o');
	const libffiArchivePath = path.join(OUTPUT_DIR, 'libffi.a');
	...
	await run(runtime.wasmLd, ['-r', '-o', prelinkedObjectPath, ...objectPaths]);
	...
	await finalizeObjectiveCReceipt({
		...
		foundationSourceCount: sources.length,
		usesLibffi: useLibffi
	});
	console.log(`[foundation-build] wrote ${archivePath}`);
	console.log(`[foundation-build] wrote ${prelinkedObjectPath}`);
	console.log(`[foundation-build] wrote ${path.join(OUTPUT_DIR, 'foundation-headers.json')}`);
}

async function prelinkExistingFoundationObjects(runtime) {
	const selectedSources = (process.env.WASM_LLVM_OBJECTIVE_C_FOUNDATION_PRELINK_SOURCES || '')
		.split(',')
		.map((source) => source.trim())
		.filter(Boolean);
	if (!selectedSources.length) {
		throw new Error('WASM_LLVM_OBJECTIVE_C_FOUNDATION_PRELINK_SOURCES is required');
	}
	const objectPaths = selectedSources.map((sourceName) =>
		path.join(
			OBJECT_DIR,
			sourceName === 'wasm_llvm_foundation_support.c'
				? 'wasm_llvm_foundation_support.o'
