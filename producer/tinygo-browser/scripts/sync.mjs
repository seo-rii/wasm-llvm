...
export async function syncEmceptionRuntime({
	...
	if (!reusedExistingWorker) {
		const banner = `/* Generated from ${workerUrl} by the wasm-llvm TinyGo producer. */\n`;
...
async function main(argv = process.argv.slice(2)) {
	const [outputPath, workerUrl] = argv;
	if (!outputPath || argv.includes('--help') || argv.includes('-h')) {
		console.log('Usage: pnpm sync:tinygo -- OUTPUT_PATH [WORKER_URL]');
		if (!outputPath) process.exitCode = 1;
		return;
	}
	const result = await syncEmceptionRuntime({ outputPath: path.resolve(outputPath), workerUrl });
	console.log(`Synchronized ${result.assetNames.length} TinyGo/emception assets to ${outputPath}`);
}
