...
const WORKFLOW_PATH = path.join(REPO_ROOT, '.github', 'workflows', 'swift-browser-producer.yml');
...
test('Swift runtime export workflow is manual and preserves verification gates', async () => {
	...
	assert.match(workflow, /pnpm --dir producer\/swift-browser run workflow:preflight/u);
	...
	assert.match(workflow, /pnpm --dir producer\/swift-browser run probe:toolchain/u);
	assert.match(workflow, /--run-wasm/u);
	assert.match(workflow, /--receipt "\$RUNNER_TEMP\/wasm-swift-toolchain-probe\.json"/u);
	assert.match(workflow, /pnpm --dir producer\/swift-browser run bootstrap:source/u);
	...
	assert.match(workflow, /pnpm --dir producer\/swift-browser run build:browser-compiler/u);
	...
	assert.match(workflow, /pnpm --dir producer\/swift-browser run discover:build-outputs/u);
	assert.match(workflow, /--build-dir "\$WASM_SWIFT_BUILD_DIR"/u);
	...
	assert.match(workflow, /pnpm --dir producer\/swift-browser run run:upstream-baseline/u);
	...
	assert.match(workflow, /pnpm --dir producer\/swift-browser run verify:build-outputs/u);
	...
	assert.match(workflow, /pnpm --dir producer\/swift-browser run package:from-plan/u);
	assert.match(workflow, /pnpm --dir producer\/swift-browser run export:runtime/u);
	...
	assert.match(workflow, /producer\/swift-browser\/out\/PROMOTE\.md/u);
	assert.match(workflow, /Upload the archive and descriptor to external static hosting/u);
	assert.match(workflow, /actions\/upload-artifact@v4/u);
	assert.match(workflow, /producer\/swift-browser\/out\/\*\.tar\.gz/u);
	assert.match(workflow, /producer\/swift-browser\/out\/\*\.sha256/u);
	assert.match(workflow, /producer\/swift-browser\/out\/\*\.json/u);
	assert.match(workflow, /producer\/swift-browser\/out\/PROMOTE\.md/u);
	assert.match(workflow, /if-no-files-found:\s+error/u);
	assert.match(workflow, /name:\s+swift-browser-compiler-diagnostics/u);
	...
	assert.match(workflow, /producer\/swift-browser\/dist\/runtime-build\.json/u);
	assert.match(workflow, /producer\/swift-browser\/dist\/runtime-manifest\.v1\.json/u);
