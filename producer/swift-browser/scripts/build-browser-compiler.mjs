...
export async function createSwiftBrowserCompilerBuildPlan({
	...
	const plan = {
		...
		browserCompilerBuild: {
			...
			note: 'Packaging accepts only these browser compiler outputs after contract validation. Native SwiftWasm SDK artifactbundles are SDK inputs, not browser compiler outputs.'
		},
		expectedOutputs: {
		...
		nextCommands: [
			...
			`pnpm --dir producer/swift-browser run run:upstream-baseline -- --plan ${normalizedPlanPath} --preset buildbot_linux_crosscompile_wasm`,
			`pnpm --dir producer/swift-browser run discover:build-outputs -- --build-dir ${normalizedBuildDir} --plan ${normalizedPlanPath} --allow-official-sdk-placeholder --write-plan`,
			`pnpm --dir producer/swift-browser run prepare:raw-runtime -- --source-dir ${normalizedRawRuntimeDir} --runner-worker <runner-worker.js> --swiftc-wasm <swiftc.wasm> --swiftpm-wasm <swiftpm.wasm> ${fetchOfficialSdk ? '--fetch-official-sdk' : '--sdk-archive <sdk.tar.gz>'}`,
			`pnpm --dir producer/swift-browser run verify:build-outputs -- --plan ${normalizedPlanPath} --require-browser-compiler-contracts --require-browser-build-command --require-browser-build-execution --require-browser-build-log --require-source-bootstrap-provenance`,
			`pnpm --dir producer/swift-browser run package:from-plan -- --plan ${normalizedPlanPath} --swift-version <version> --wasm-sdk-id <sdk-id> --source "<build provenance>" --browser-contract`,
...
function usage() {
	return [
		'Usage: pnpm --dir producer/swift-browser run build:browser-compiler -- --checkout-root path/to/swift-source-root [options]',
		'',
