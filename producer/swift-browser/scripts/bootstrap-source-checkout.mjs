...
const DEFAULT_SWIFT_REPOSITORY = PRODUCER_MANIFEST.sources.swift.repository;
const DEFAULT_SWIFT_REF = PRODUCER_MANIFEST.sources.swift.ref;
const DEFAULT_DEPENDENCY_SCHEME = PRODUCER_MANIFEST.sources.dependencyScheme;
...
export async function createSwiftSourceBootstrapPlan({
	...
	const plan = {
		...
		nextCommands: [
			`pnpm --dir producer/swift-browser run build:browser-compiler -- --checkout-root ${normalizedSourceRoot} --browser-build-command "<command that creates runner-worker.js swiftc.wasm swiftpm.wasm>" --allow-missing-tools`,
			'pnpm --dir producer/swift-browser run prepare:raw-runtime -- --source-dir <raw-runtime> --runner-worker <runner-worker.js> --swiftc-wasm <swiftc.wasm> --swiftpm-wasm <swiftpm.wasm> --fetch-official-sdk'
		]
	};
...
function usage() {
	return [
		'Usage: pnpm --dir producer/swift-browser run bootstrap:source -- [options]',
		'',
